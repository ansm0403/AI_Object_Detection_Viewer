...
**AGENTS.md (this file) is the single entry point.** New contexts read this first;
...
| Group | Files | Role |
|---|---|---|
| **Spec triangle** (the design itself) | `docs/PROJECT_DESIGN.md` + `.Codex/docs/architecture.md` + `.Codex/docs/mvp-checklist.md` | why + current structure + step-by-step progress |
| **Rules + entry point** (always loaded) | `AGENTS.md` | Immutable Rules, Workflow Rules, Error Defaults, Testing Policy |
| **Reading aids** (consulted as needed) | `.Codex/docs/domain-glossary.md` + `docs/edgecases/Edge_#N.md` | terminology + per-Step decision history |

Detail per file:

- `.Codex/docs/architecture.md` — current spec: data model, folder structure, data flow,
  component contracts, validation rules. Updated each Step.
- `.Codex/docs/mvp-checklist.md` — step-by-step progress, decisions, and per-Step Edge case
  references. Updated each Step.
- `.Codex/docs/domain-glossary.md` — 3D/AI term definitions. Read before explaining domain
